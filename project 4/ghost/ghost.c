...
int main(int argc, char *argv[])
{
    ...
    //MPI_Irecv(&data[(DOMAINSIZE - 1) * DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_top, 0, comm_cart, &request);
    MPI_Irecv(&data[(DOMAINSIZE - 1) * DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0, comm_cart, &request);

    //MPI_Send(&data[DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0, comm_cart);
    ...
    MPI_Send(&data[(DOMAINSIZE - 2) * DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 1, comm_cart);

    MPI_Wait(&request, &status);

    //  to the left

    MPI_Irecv(&data[2 * DOMAINSIZE - 1], 1, data_ghost, rank_right, 2, comm_cart, &request);
    ...
    //  to the right
    MPI_Irecv(&data[DOMAINSIZE], 1, data_ghost, rank_left, 3, comm_cart, &request);

    MPI_Send(&data[2 * DOMAINSIZE - 2], 1, data_ghost, rank_right, 3, comm_cart);
